...
function ensureSchema() {
  if (!schemaPromise) {
    ...
    schemaPromise = (async () => {
      ...
      // Per-employee saved contractor names, for autocomplete. Scoped to the
      // owner so one person's list never mixes with another's. The lower(name)
      // unique index makes saves case-insensitive ("JT Build" == "jt build").
      await sql`
      ...
      `;
      await sql`CREATE UNIQUE INDEX IF NOT EXISTS contractors_owner_name ON contractors (owner_id, lower(name))`;
      // Photo column upgrade (kept from earlier) below.
...
// ── Saved contractors (per employee) ──

export async function getContractors(ownerId) {
  await ensureSchema();
  const sql = db();
  const rows = await sql`
    SELECT name FROM contractors WHERE owner_id = ${ownerId} ORDER BY lower(name) ASC
  `;
  return rows.map((r) => r.name);
}

export async function addContractor(ownerId, name) {
  await ensureSchema();
  const sql = db();
  await sql`
    INSERT INTO contractors (owner_id, name) VALUES (${ownerId}, ${name})
